...
class ProductBloc extends Bloc<ProductEvent, ProductState> {
  ...
  Future<void> _onFetchProducts(FetchProductsEvent event, Emitter<ProductState> emit) async {
    if (event.isRefresh) {
      ...
      if (state is ProductsLoaded && (state as ProductsLoaded).hasReachedMax) return;
      if (state is! ProductsLoaded) emit(ProductLoading());
    }

    try {
      final page = event.isRefresh ? 1 : _currentPage;
      final newProducts = await getProductsUseCase(GetProductsParams(page: page, pageSize: _pageSize));
      ...
      if (state is ProductsLoaded && !event.isRefresh) {
        ...
        emit(ProductsLoaded(newProducts, hasReachedMax: newProducts.length < _pageSize));
      }
  ...
  Future<void> _onSearchProducts(SearchProductsEvent event, Emitter<ProductState> emit) async {
    emit(ProductLoading());
    try {
      final products = await searchProductsUseCase(SearchProductsParams(query: event.query));
      emit(ProductsLoaded(products, hasReachedMax: true)); // Search results don't paginate typically here
    } catch (e) {
      emit(ProductError(e.toString()));
    }
  }
